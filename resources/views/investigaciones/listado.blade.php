        ...
        <div class="row">
            <div class="col-12 my-3">
                @can('invetigacion.filtro.todas')
                    <a href="/investigacionesTodas/" class="btn btn-dark m-1" rel="noopener noreferrer">Todas
                    @endcan
                    @can('invetigacion.filtro.pendiente')
                        <a href="/investigacionesTodas/17" class="btn btn-dark m-1" rel="noopener noreferrer">Pendiente de
                            aprobación</a>
                    @endcan
                    @can('invetigacion.filtro.devuelto')
                        <a href="/investigacionesTodas/19" class="btn btn-dark m-1" rel="noopener noreferrer">Devuelto</a>
                    @endcan
                    @can('invetigacion.filtro.canceladoColpensiones')
                        <a href="/investigacionesTodas/20" class="btn btn-dark m-1" rel="noopener noreferrer">Cancelado
                            colpensiones</a>
                    @endcan
                    @can('invetigacion.filtro.solicitado')
                        <a href="/investigacionesTodas/3" class="btn btn-dark m-1" rel="noopener noreferrer">Solicitado</a>
                    @endcan
                    @can('invetigacion.filtro.asignado')
                        <a href="/investigacionesTodas/5" class="btn btn-dark m-1" rel="noopener noreferrer">Asignado</a>
                    @endcan
                    @can('invetigacion.filtro.revision')
                        <a href="/investigacionesTodas/6" class="btn btn-dark m-1" rel="noopener noreferrer">En
                            revisión</a>
                    @endcan
                    @can('invetigacion.filtro.finalizado')
                        <a href="/investigacionesTodas/7" class="btn btn-dark m-1" rel="noopener noreferrer">Finalizado</a>
                    @endcan
                    @can('invetigacion.filtro.objetado')
                        <a href="/investigacionesTodas/8" class="btn btn-dark m-1" rel="noopener noreferrer">Objetado</a>
                    @endcan
                    @can('invetigacion.filtro.canceladoJavh')
                        <a href="/investigacionesTodas/9" class="btn btn-dark m-1" rel="noopener noreferrer">Cancelado
                            javh</a>
                    @endcan
                    @can('invetigacion.filtro.correccion')
                        <a href="/investigacionesTodas/11" class="btn btn-dark m-1" rel="noopener noreferrer">En
                            Corrección</a>
                    @endcan
                    @can('invetigacion.filtro.aprobadoObjetado')
                        <a href="/investigacionesTodas/21" class="btn btn-dark m-1" rel="noopener noreferrer">Aprobado
                            Objetado</a>
                    @endcan
                    @can('invetigacion.filtro.objetadoFinalizado')
                        <a href="/investigacionesTodas/16" class="btn btn-dark m-1" rel="noopener noreferrer">Objetado
                            Finalizado</a>
                    @endcan
            </div>
                        ...
                        <table id="investigacionesTable" class="table  table-responsive table-striped">
                            <thead class="bg-primary text-white">
                                <tr>
                                    ...
                                    <th>Prioridad</th>
                                    <th>Ciudad</th>
                                    @if (Auth::user()->id == 1 || Auth::user()->centroCosto == 1)
                                        <th>Coor. Regional</th>
                                        <th>Investigador</th>
                                        <th>Analista</th>
                                    @endif
                            ...
                            <tbody>
                                @forelse ($investigaciones as $investigacion)
                                    ...
                                    @if (Auth::user()->id == 1 || Auth::user()->centroCosto == 1)
                                        ...
                                        <td class="align-middle text-capitalize">
                                            {{ $investigacion->name_investigador ? $investigacion->name_analista : 'No asignado' }}
